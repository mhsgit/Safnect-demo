...
object BLEByteArrayUtil {
    ...
    fun calcSendbyteArray(byteArray: ByteArray): Array<ByteArray?> {
        PeachLogger.d(GlobalConstant.APP_TAG, "${ServerFragment.TAG} calcSendbyteArray 原始数据长度byteArray = ${byteArray.size}")
        //根据当前的传输MTU值计算要分的包数
        ...
        PeachLogger.d(GlobalConstant.APP_TAG, "${ServerFragment.TAG} calcSendbyteArray 总包数totalpkgs = ${totalpkgs}，包大小everybytelen = $everybytelen")
