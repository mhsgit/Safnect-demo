package com.populstay.wallet.repository
...
object BlueToothBLEUtil {
    ...
    suspend fun writeCharacteristicSplit(
        ...
        PeachLogger.d(GlobalConstant.APP_TAG, " writeCharacteristicSplit characteristic = ${characteristic.uuid}, 发送数据totalSize = ${byteArray.size}")
        if (checkBlueToothPermission(Manifest.permission.BLUETOOTH_CONNECT)) {
            mBluetoothGatt?.let {
                //获取到拆分后的数据包数组
                val sendarray = BLEByteArrayUtil.calcSendbyteArray(byteArray)
                for (cursend in sendarray) {
                    PeachLogger.d(GlobalConstant.APP_TAG, " writeCharacteristicSplit characteristic = ${characteristic.uuid}, 发送数据 size = ${cursend?.size}, cursend = ${cursend?.toString(
                        Charsets.UTF_8)}")
                    cursend?.let { cur ->
                        if (characteristic.uuid == BlueToothBLEUtil.getUUID(BlueToothBLEUtil.BLECHARACTERISTIC_NEW_MESSAGE_WRITE)){
                            PeachLogger.d(GlobalConstant.APP_TAG, "数据通信-->主端发出-->cur=${CommonUtil.calculateMD5(cur)}，size=${cur.size}")
                        }
                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                            it.writeCharacteristic(
                                characteristic,
    ...
    fun dealRecvByteArray(deviceAdr: String, value: ByteArray): Boolean {
        ...
        PeachLogger.d(GlobalConstant.APP_TAG, "dealRecvByteArray=${CommonUtil.calculateMD5(value)}，size=${value.size}--totalpkgs=$totalpkgs--curpkgs=$curpkgs--curdatabytes=${CommonUtil.calculateMD5(curdatabytes)}")
        ...
        if (mHtRecvByteArray.contains(deviceAdr)){
            ...
            arraybytes?.let {
                it[curpkgs]?.let {
                    PeachLogger.d(GlobalConstant.APP_TAG, "dealRecvByteArray有重复数据包=${CommonUtil.calculateMD5(value)}，size=${value.size}")
                    return false
                }
            }
        }
    ...
    fun setCharacteristicNotify(characteristic: BluetoothGattCharacteristic, bool: Boolean) {
        if (checkBlueToothPermission(Manifest.permission.BLUETOOTH_CONNECT)) {
            mBluetoothGatt?.let {
                ...
                PeachLogger.d(GlobalConstant.APP_TAG, "${ConnectFragment.TAG} setCharacteristicNotify = $enable , uuid = ${characteristic.uuid}")
            }
        }
    }
